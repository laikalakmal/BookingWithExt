...
namespace Core.Application.Mappings
{
    public class TourPackageMapper : IProductMapper<TourPackage, TourPackageDto>
    {
        public static TourPackageDto FromDomain(Product product)
        {
            ...
            {
                throw new InvalidOperationException($"Failed to map TourPackage to DTO: Null reference encountered. Property: {ex.Message}", ex);
            }
            catch (Exception ex)
            {
                throw new InvalidOperationException($"Failed to map TourPackage to DTO: {ex.Message}", ex);
            }
        }
        ...
        public static TourPackage ToDomain(ProductDto productDto)
        {
            ...
            {
                throw new InvalidOperationException($"Failed to create TourPackage domain entity: Null reference encountered. Property: {ex.Message}", ex);
            }
            catch (ArgumentException ex)
            {
                throw new InvalidOperationException($"Failed to create TourPackage domain entity: Invalid argument. {ex.Message}", ex);
            }
            catch (Exception ex)
            {
                throw new InvalidOperationException($"Failed to create TourPackage domain entity: {ex.Message}", ex);
            }
        }
    }
}
